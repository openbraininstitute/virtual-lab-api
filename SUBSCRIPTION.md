...
```mermaid
graph TD
    A[User Creates Virtual Lab] --> B[Free Subscription Activated]
    B --> C{Choose Subscription}
    C -->|Upgrade| D[Pro/Premium Plan]
    C -->|Stay on| E[Free Plan]
    D --> F[Access Advanced Features]
    D --> G[Receive Monthly/Yearly Credits]
    E --> H[Limited Features]
    E --> I[Basic Credits]
    D -->|Top-up| J[Add Additional Credits]
    E -->|Top-up| J
    D -->|Payment Fails| E
```
...
| Plan | Features | Monthly Credits | Yearly Credits |
|------|----------|----------------|----------------|
...
- **Free Plan**: Users receive 100 credits once when creating a virtual lab
...
- **Conversion Rate**: Money paid for subscriptions or top-ups is converted to credits (at a rate of 0.5 credits per monetary unit)
...
A subscription can exist in various states throughout its lifecycle:

- **Active**: Subscription is in good standing with successful payments
- **Past Due**: Recent payment failed but subscription still active
- **Unpaid**: Payment failed and subscription is no longer active
- **Canceled**: Subscription has been explicitly canceled
- **Incomplete**: Subscription has not been fully created yet
- **Incomplete Expired**: Initial payment failed and subscription was not created
...
stateDiagram-v2
    ...
    Incomplete --> IncompleteExpired: Payment Fails
    Active --> PastDue: Payment Fails
    PastDue --> Active: Payment Succeeds
    PastDue --> Unpaid: Multiple Failures
...
- Regular background jobs verify consistency
