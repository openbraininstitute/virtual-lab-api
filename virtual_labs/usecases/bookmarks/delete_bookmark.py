...
async def delete_bookmark(
    db: AsyncSession, project_id: UUID4, resource_id: str, category: BookmarkCategory
) -> None:
    ...
    try:
        ...
        logger.error(
            f"DB error during deleting bookmark for resource {resource_id} - {category} from project {project_id}: ({error})"
        )
        raise VliError(
        ...
        logger.exception(
            f"Error during deleting bookmark for {resource_id} - {category} from project {project_id}: ({error})"
        )
        raise VliError(
