...
async def retrieve_all_user_projects_use_case(
    ...
    try:
        project_ids = user.grants.projects.all
            ...
            ProjectVlOut.model_validate(
                {
                    **p.__dict__,
                    ...
                    "is_waitlisted": p.id in user.grants.projects.waitlisted,
                }
            )
            for p, _ in results.rows
        ]

        recent_workspace = await prefr.get_user_recent_workspace(user.id)
        ...
        logger.error(f"Error during retrieving project for user {user.id}: ({ex})")
        raise VliError(
