...
from virtual_labs.usecases.labs._user_labs_helpers import enrich_vlab
...
async def get_my_virtual_lab_use_case(
    *,
    ...
    user, _token = auth
    try:
        owned = await repository.get_user_virtual_lab(db=session, owner_id=user.id)
    ...
    if owned is None:
        return None
    return await enrich_vlab(owned, ProjectQueryRepository(session=session))
