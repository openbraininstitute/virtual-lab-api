...
async def rate_limit_initiate(
    ...
    if count is None:
        await rl.set(key, 1, ttl=settings.LOCK_TIME_SECONDS)  # 1h
    elif count >= settings.MAX_INIT_ATTEMPTS:
...
async def rate_limit_verify(
    ...
    """Enforce rate limit for verifying email codes (e.g. 3/1h)."""
