...
def upgrade() -> None:
    ...
    # Add budget_amount column and set default value using raw SQL
    op.add_column(
        "virtual_lab", sa.Column("budget_amount", sa.Integer(), nullable=True)
    )
    op.execute("UPDATE virtual_lab SET budget_amount = 0")
    op.alter_column("virtual_lab", "budget_amount", nullable=False)
